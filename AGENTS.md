...
- MOTD behavior is controlled by `--motd keep`, `--motd replace`, or `--motd combine`. `replace` is the noninteractive default and should back up and hide a regular static `/etc/motd` plus other executable update-motd entries before showing linux-cli-setup only. `keep` should leave the existing MOTD alone, remove linux-cli-setup login hooks, and retain refreshed inactive MOTD assets so `motdreplace` can enable them later. `combine` should show the existing MOTD first, then linux-cli-setup. If no MOTD option or `LINUX_CLI_MOTD_MODE` value is provided in an interactive install and no saved mode exists, prompt for the mode. Update should reuse `motd_mode` from install state unless the user passes a new mode and preserve the chosen enablement mode. For all modes, update must refresh the versioned MOTD executable and managed UniFetch MOTD configuration; replace/combine modes must also refresh the login hook. Back up a changed UniFetch config before installing the new managed version. The installed MOTD should prefer UniFetch with OS-matched ASCII art, then fall back to the built-in linux-cli-setup status block when it is unavailable or fails. Built-in and legacy-compatibility status blocks must align every labeled value to the same column, including storage rows. Both paths should use separated system, resource, network, and service groups and show OS and kernel details, uptime, load, available temperature, process and logged-in-user counts, memory usage with percentage, swap usage, distinct mounted local filesystems, currently mounted NFS filesystems, remaining storage, local IP, configured DNS servers, internet/public-IP status, cached package-update status, UFW and SSH status, and reboot requirement. Exclude mountpoints beginning with `/snap`. Do not activate dormant automounts or perform a package metadata refresh from the MOTD. Keep network, storage, and local status probes bounded by short timeouts so login remains responsive. Arch/Garuda may install optional UniFetch through pacman or yay/AUR. Debian/Ubuntu must not add a third-party UniFetch repository.
