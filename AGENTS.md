...
- `scripts/utilities/` contains managed utility commands copied to `/usr/local/bin`, including `aliases`, `timecheck`, `updatecheck`, `internetcheck`, `needs-reboot`, `motdreplace`, `lcsversion`, and `dockercheck`.
- `templates/fish/` contains Fish, Fisher, Tide, abbreviation, fallback MOTD hook, and Fastfetch MOTD block templates.
...
- `tests/test-fastfetch-motd.sh` verifies Fastfetch selection, Garuda defaults, package enforcement, and managed Fish block append, idempotence, removal, and user-content preservation without changing the host.
...
`install.sh` always includes `core`; optional profiles are selected with `--profile`, `--profiles`, or `--all-profiles`. If saved install state or a linux-cli-setup-specific legacy managed-file marker exists, `install.sh` must exit near the start and direct the operator to `update.sh`. If neither state nor a managed marker exists, `update.sh` must exit and direct the operator to `install.sh`.
...
- Fresh install should back up existing user Fish files before replacement unless the installed file already matches the project template. Update must preserve host and user configuration files byte-for-byte and install only missing structured configuration files, except that Fastfetch MOTD mode may append or remove its precisely marked managed block in the target user's `config.fish`. The versioned UniFetch MOTD config is an app asset and must refresh with the MOTD executable, with changed prior copies backed up. Only safely parsed, simple key/value files such as `/etc/auto-update.conf` may receive missing settings additively; never otherwise line-merge structured Fish, SSH, systemd, chrony, MOTD, sysctl, firewall, or hardening configuration.
- Fish should become the target user's default shell through `/etc/shells` plus `chsh` or `usermod`.
- Install state belongs in `/var/lib/linux-cli-setup/install.env`; preserve the originally saved shell across updates and store the installed project version as `version=`. Treat that state value as authoritative when comparing the installed version with the running update target. Also install the version at `/usr/local/share/linux-cli-setup/VERSION` as a fallback for `lcsversion`. Use the state file as the primary installation marker, with project-specific MOTD, fail2ban, logrotate, sysctl, and installed-version files as legacy detection fallbacks.
...
Fastfetch remains a recommended core package during normal installs. When `--motd fastfetch` is selected or restored from saved state, it becomes required and its command must be verified before MOTD configuration changes.
...
- Install regular files from `scripts/utilities/` into `/usr/local/bin` as root-owned executable utility commands during both install and update. Uninstall should remove matching managed utility commands by comparing them against the repository copies and should back up changed local files instead of deleting them, except `lcsversion` remains so it can report the uninstalled state. The `aliases` utility must print all Fish abbreviations and aliases visible to the current user. The `timecheck` utility must show chrony/NTP status, selected time source, and stratum; install `/usr/local/bin/ntpcheck` as a managed alias symlink to `timecheck`. `updatecheck` must list OS package updates and obtain confirmation before installing them. `internetcheck` must use bounded probes for external IP, fping latency, configured DNS-server reachability and dig latency, and Cloudflare throughput using approximately 50 MB down and 20 MB up. `needs-reboot` must return `1` when a Debian/Ubuntu reboot marker exists or a monitored kernel/core system component was updated after boot. `motdreplace` must be root-only, safely replace competing login MOTD entries, and atomically save `motd_mode=replace` without discarding other install state. The Docker utility is named `dockercheck`.
- MOTD behavior is controlled by `--motd keep`, `--motd replace`, `--motd combine`, or `--motd fastfetch`. `replace` is the general noninteractive default and should back up and hide a regular static `/etc/motd` plus other executable update-motd entries before showing linux-cli-setup only. `keep` should leave the existing MOTD alone, remove linux-cli-setup login hooks, and retain refreshed inactive MOTD assets so `motdreplace` can enable them later. `combine` should show the existing MOTD first, then linux-cli-setup. `fastfetch` must be offered or accepted only when the target command exists or its package is available; when selected, install it as required, verify the command, use replace semantics for competing MOTDs, retire linux-cli-setup/UniFetch hooks and assets, and show only the marked Fastfetch Fish block. Garuda Linux defaults to fastfetch only when no explicit or saved mode exists. Explicit install choices override that default. Updates must reuse the saved `motd_mode` unless `update.sh --motd MODE` explicitly changes it; changing away from fastfetch removes only the managed block. For non-Fastfetch replace/combine modes, update must refresh the versioned MOTD executable and managed UniFetch MOTD configuration and refresh the login hook. Back up a changed UniFetch config before installing the new managed version. The installed MOTD should prefer UniFetch with OS-matched ASCII art, then fall back to the built-in linux-cli-setup status block when UniFetch is unavailable or fails. Built-in and legacy-compatibility status blocks must align every labeled value to the same column, including storage rows. Both paths should use separated system, resource, network, and service groups and show OS and kernel details, uptime, load, available temperature, process and logged-in-user counts, memory usage with percentage, swap usage, distinct mounted local filesystems, currently mounted NFS filesystems, remaining storage, local IP, configured DNS servers, internet/public-IP status, cached package-update status, UFW and SSH status, and reboot requirement. Exclude mountpoints beginning with `/snap`. Do not activate dormant automounts or perform a package metadata refresh from the MOTD. Keep network, storage, and local status probes bounded by short timeouts so login remains responsive. Arch systems may install optional UniFetch through pacman or yay/AUR. Debian/Ubuntu must not add a third-party UniFetch repository.
...
```bash
for file in install.sh update.sh uninstall.sh install_test.sh scripts/setup-linux-cli.sh scripts/uninstall-linux-cli.sh scripts/install-test-linux-cli.sh scripts/lib/linux-cli-common.sh scripts/lib/package-install-overrides.sh scripts/utilities/* templates/motd/linux-cli-motd templates/motd/unifetch-motd.conf templates/bin/time-status templates/bin/ntp-status templates/auto-update/auto-update templates/auto-update/auto-update.conf templates/chrony/chrony-dhcp-source templates/chrony/networkmanager-dispatcher templates/chrony/dhclient-exit-hook tests/test-fastfetch-motd.sh; do bash -n "$file"; done
for file in templates/fish/config.fish templates/fish/configure_tide.fish templates/fish/fastfetch-motd.fish templates/fish/conf.d/linux-cli-motd.fish templates/fish/functions/*.fish; do fish -n "$file"; done
shellcheck install.sh update.sh uninstall.sh install_test.sh scripts/setup-linux-cli.sh scripts/uninstall-linux-cli.sh scripts/install-test-linux-cli.sh scripts/lib/linux-cli-common.sh scripts/lib/package-install-overrides.sh scripts/utilities/* templates/motd/linux-cli-motd templates/motd/unifetch-motd.conf templates/bin/time-status templates/bin/ntp-status templates/auto-update/auto-update templates/auto-update/auto-update.conf templates/chrony/chrony-dhcp-source templates/chrony/networkmanager-dispatcher templates/chrony/dhclient-exit-hook tests/test-fastfetch-motd.sh
./tests/test-fastfetch-motd.sh
